...
def sequence_mask(lens, max_len=None):
    ...
    if lens.data.is_cuda:
        ranges = ranges.cuda(lens.device)
...
class CRF(nn.Module):
    ...
    def calc_binary_score(self, labels, lens):
        ...
        mask = sequence_mask(lens + 1, seq_len+1).float()
    ...
    def calc_unary_score(self, logits, labels, lens):
        ...
        mask = sequence_mask(lens, scores.size()[-1]).float()
        scores = scores * mask
        return scores
...
# lens = torch.tensor([2,1]) # 2, 3
# labels = torch.tensor([[1,0,0],[0, 1, 1]]) # 2, 3
#
# crf = CRF(4)
# logits = crf.pad_logits(logits)
# norm_score = crf.calc_norm_score(logits, lens)
# print(norm_score)
# gold_score = crf.calc_gold_score(logits, labels, lens)
# print(gold_score)
# loglik = gold_score - norm_score
# print(loglik)
# print(crf.viterbi_decode(logits, lens))