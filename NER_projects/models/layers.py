...
class InteractAttention(nn.Module):
    def __init__(self, d_k, dropout=.1):
        ...
        self.scale_factor = np.sqrt(d_k)
        self.softmax = nn.Softmax(dim=-1)
        self.interact_matrix = nn.Parameter(torch.FloatTensor(d_k,d_k))
        self.dropout = nn.Dropout(dropout)

        init.xavier_uniform_(self.interact_matrix)

    def forward(self, q, k, v, attn_mask=None):
        # q: [b_size x len_q x d_k]
        # k: [b_size x len_k x d_k]
        # v: [b_size x len_v x d_v] note: (len_k == len_v)
        attn = q.bmm(self.interact_matrix).bmm(k.transpose(1, 2)) / self.scale_factor  # attn: [b_size x len_q x len_k]
        if attn_mask is not None:
            assert attn_mask.size() == attn.size()
            attn.data.masked_fill_(attn_mask, -float('inf'))
...
class _MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(-2).repeat(n_heads, seq_len, 1) == 0
...
class LabelSmoothing(nn.Module):
    ...
    def __init__(self, num_labels, smoothing=0.0):
    ...
    def forward(self, logits, labels, predict_mark):
        '''

        :param x: batch_size, seq_len, num_labels
        :param labels:batch_size, seq_len
        :param predict_mark:batch_size, seq_len
        :return:KLdiv loss
        '''
        x = torch.nn.functional.log_softmax(logits,dim=-1)
        true_dist = x.detach().clone().fill_(self.smoothing / (self.num_labels-1))
        true_dist.scatter_(-1, labels.unsqueeze(-1), self.confidence)
        true_dist[predict_mark==0] = 0
        self.true_dist = true_dist
        return self.criterion(x, true_dist)

# torch.manual_seed(2)
# logits = torch.tensor([[[0,1], [1,0], [1,0]], [[1,0], [0,1], [2.2,-2.1]]]) # 2, 3, 2
# predict_mask = torch.tensor([[1,1,1],[1,1,0]]) # 2, 3
# labels = torch.tensor([[1,0,0],[0, 1, 1]]) # 2, 3
#
# crit = LabelSmoothing(2,0.1)
# print(crit(logits, labels, predict_mask))