...
class SoftmaxDecoder(nn.Module):
    ...
    def forward(self, inputs, predict_mask, label_ids=None):
        ...
        if label_ids is not None:
            ...
            #return torch.argmax(logits, -1), torch.argmax(p, -1).sum(-1)/lens.float() # use the sum of predict probs as sequence score
...
class CRFDecoder(nn.Module):
    ...
    def forward(self, inputs, predict_mask, labels=None):
        ...
        if labels is None:
            ...
            #return preds, scores/lens.float()
            return preds, p
        return self.loss(logits, lens, labels)

    def loss(self, logits, predict_mask, labels):
