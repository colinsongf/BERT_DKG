...
def train():
    ...
    for epoch in trange(start_epoch, config['train']['epochs'], desc="Epoch"):
        ...
        logger.info("***** Running training epoch %d*****" % (epoch+1))
        ...
        if config['train']['bootstrapping'] and (epoch+1)>3:
            ...
            scores_list = torch.tensor([]).to(device)
            outputs_list = torch.LongTensor([]).to(device)
            for batch in tqdm(unlabeled_data_loader, desc="Predicting on unlabeled train set"):
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_mask, segment_ids, predict_mask, _ = batch
                with torch.no_grad():
                    outputs, p = model(input_ids, segment_ids, input_mask, predict_mask)
                    scores = -((p*p.log()).sum(-1)*predict_mask.float()).sum(-1)
                    scores_list = torch.cat([scores_list, scores])
                    outputs_list = torch.cat([outputs_list, outputs])
            logger.info(scores_list.__str__())
            # method 1
            # good_predict = scores_list>threshold
            # good_features = np.array(unlabeled_train_features)[good_predict.cpu().numpy() == 1]
            # bad_features = np.array(unlabeled_train_features)[good_predict.cpu().numpy() == 0]
            # label_ids = outputs_list[good_predict]

            # method 2
            # top_arg = 0.01
            # sorted_scores = scores_list.argsort(0,descending=True).cpu()
            # part_value = int(scores_list.size()[0]*top_arg)
            # good_features = np.array(unlabeled_train_features)[sorted_scores[:part_value]]
            # bad_features = np.array(unlabeled_train_features)[sorted_scores[part_value:]]
            # label_ids = outputs_list[sorted_scores[:part_value]]

            # method 3
            top_arg = 0.05
            sorted_scores = scores_list.argsort(0, descending=True).cpu()
            part_value = int(len(train_features) * top_arg)
            good_features = np.array(unlabeled_train_features)[sorted_scores[:part_value]]
            bad_features = np.array(unlabeled_train_features)[sorted_scores[part_value:]]
            label_ids = outputs_list[sorted_scores[:part_value]]

            for i, feature in enumerate(good_features):
                feature.label_ids = label_ids[i].tolist()
            train_features.extend(good_features.tolist())
            unlabeled_train_features = bad_features.tolist()
            logger.info("\nIn epoch %d, %d/%d unlabeled train data put into train set" % (
                epoch + 1, len(good_features), scores_list.size()[0]))
