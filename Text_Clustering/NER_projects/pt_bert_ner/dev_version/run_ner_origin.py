...
class CONLLProcessor(DataProcessor):
    ...
    def get_unlabeled_train_examples(self, data_dir):
        return DataProcessor.create_examples_from_conll_format_file(os.path.join(data_dir, UNLABELED_TRAIN+'.txt'), 'train_unlabeled')
    
    @staticmethod
    def get_labels():
        #return ['X', 'O', 'B-PER', 'I-PER', 'B-ORG', 'I-ORG', 'B-LOC', 'I-LOC', 'B-MISC', 'I-MISC']
        return ['X', 'O', 'B-FIELD', 'I-FIELD', 'B-TEC', 'I-TEC', 'B-MISC', 'I-MISC']
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and os.path.exists(sys.argv[1]):
        ...
        config['task']['output_dir'] = config['task']['output_dir'] + "_ori"
