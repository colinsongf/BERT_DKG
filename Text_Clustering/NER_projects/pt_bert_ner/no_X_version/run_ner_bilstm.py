...
class BilstmNER(nn.Module):
    ...
    def __init__(self, config, num_labels, decoder):
        ...
        self.embedding = torch.nn.Parameter(torch.tensor([config.vocab_size, self.hidden_size]), dtype=torch.float32)
    ...
    def forward(self, input_ids, segment_ids, input_mask, predict_mask, label_ids=None):
        ...
        hidden = self.bilstm(embed)
        return self.decoder(hidden, predict_mask, label_ids)

    def _get_config(self, archive_file):
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and os.path.exists(sys.argv[1]):
        ...
        if config['task']['checkpoint'] or ckpts:
            ...
            model = BilstmNER(config['task']['bert_model_dir'],
            ...
            model = BilstmNER(config['task']['bert_model_dir'], num_labels=len(label_list),
